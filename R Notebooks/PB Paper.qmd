...
df_key <- extra_data %>% 
  select(PartJD, MCAStab, MCALoc, MCAOC)
...
# Measuring Post Hoc Power (Packages: pwr)
...
pwr::pwr.chisq.test(w = .217, N = N, df = a)
...
a <- (length(unique(PB$MC_level))-1)*(length(unique(PB$Grip_level))-1)
  
pwr::pwr.chisq.test(w = .226, N = N, df = a)
